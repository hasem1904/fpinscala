...
import com.fpinscala.ch13.IO1.IO
...
object ImperativeAndLazyIO {
  ...
  /**
    * The above code is rather low-level, and it's not compositional, either.
    * Consider the following scenarios:
    * Check whether the number of _nonempty_ lines in the file exceeds 40,000
...
object SimpleStreamTransducers {
  ...
  /**
    ...
    * stream transducers. It can be in of three states:
    *
    * 1. it can be emitting a value to the output (`Emit`),
  ...
  sealed trait Process[I, O] {
    ...
    def apply(s: Stream[I]): Stream[O] = this match {
      case Halt() => Stream()
      case Await(recv) => s match { //recv: Request value from stream
    ...
    def repeat: Process[I, O] = {
      //Auxiliary recursive loop method.
...
`Process` to a `Sink`.
     ...
     */
    //import com.fpinscala.ch13.IO0.fahrenheitToCelsius
...
input (`Await`) or signaling termination via `Halt`.
  ...
  object Process {
    ...
    def takeWhile[I](f: I => Boolean): Process[I, I] = ??? // Process forms a monad, and we provide monad syntax for it
    def monad[I]: Monad[({type f[x] = Process[I, x]})#f] =
      new Monad[({type f[x] = Process[I, x]})#f] {
        def unit[O](o: => O): Process[I, O] = emit(o)
