package com.fpinscala.ch15

import com.fpinscala.ch13.IO1._
import com.fpinscala.ch13._
...
object ImperativeAndLazyIO {
  ...
  /**
    ...
    * 40,000. Find a line index before 40,000 where the first letter of
    ...
    * and we could assemble functionality using all the `Stream` functions we know and love.
    */
  object Examples {
...
object SimpleStreamTransducers {
  ...
  /**
    ...
    * 1. emitting a value to the output (`Emit`),
    * 2. reading a value from its input (`Await`) or
    * 3. signaling termination via `Halt`.
    */
  sealed trait Process[I, O] {
    ...
    def apply(s: Stream[I]): Stream[O] = this match {
      case Halt() => Stream() //A stream consisting of given element, a sequence of values of type `O`
      case Await(recv) => s match { //Request value from stream
    ...
    /**
      * Exercise 15.5:
      * Implement `|>`. Let the types guide your implementation.
      */
  ...
  object Process {
    ...
    /** Request a value (recv) from the input stream. */
    case class Await[I, O](recv: Option[I] => Process[I, O]) extends Process[I, O]

    /** Indicates to the driver that no more elements should be read from the input stream or emittet to the output stream. */
    ...
    /**
      * As an example of `repeat`, here's a definition of `filter` that uses `repeat`.
      */
    ...
    /**
      * Exercise 15.1:
      * Implement `take`, `drop`, `takeWhile`, and `dropWhile`.
      * The take function halts the Process (stream process) after it encounters the given number of elements.
      */
    def take[I](n: Int): Process[I, I] =
      if (n <= 0) Halt()
      else await(i => emit(i, take[I](n - 1))) //emit next element in the stream

    /**
      * The drop function ignores the given number of arguments and the emits/produce the rest of the stream values.
      */
    def drop[I](n: Int): Process[I, I] =
      if (n <= 0) id //Calling the identity process, just repeatedly echos its input.
      else await(i => drop[I](n - 1))

    /**
      * The takeWhile takes elements from the stream as long as the given predicate is true.
      */
    ...
    /**
      * The dropWhile drops elements from the stream as long as the given predicate is true.
      */
    def dropWhile[I](f: I => Boolean): Process[I, I] =
      await(i =>
        if (f(i)) dropWhile((f)) //drops elements from the stream
        else emit(i, id)) //or else emit the element from the stream
    ...
    /**
      * Exercise 15.2:
      * Implement `count`.
      * It should emit the number of elements seen so far. For instance
      * count(Stream("a", "b", "c")) should yield Stream(1,2,3), or Stream(0,1,2,3).
      *
      * Here's one implementation, with three stages:
      * 1. we map all inputs to 1.0,
      * 2. compute a running sum,
      * 3. then finally convert the output back to `Int`.
      *
      * The three stages will be interleaved - as soon
      * as the first element is examined, it will be converted to 1.0,
      * then added to the running total, and then this running total
      * will be converted back to `Int`.
    ...
    def count2[I]: Process[I, Int] = {
      //Auxiliary (inner) method for looping/map over the stream.
    ...
    /**
      * Exercise 15.4:
      * Implement `sum` and `count` in terms of `loop`
      */
    ...
    /**
      * Exercise 15.7:
      * Can you think of a generic combinator that would
    ...
    /**
      * Exercise 15.9:
      * Write a program that reads degrees fahrenheit as `Double` values from a file,
...
object GeneralizedStreamTransducers {
  ...
  /**
    ...
    * forceful termination, indicated by the special exception `Kill`, or some other error.
    *
  ...
  object Process {
    ...
    /**
      * Exercise 15.12:
      * Implement `join`.
      * Notice this is the standard monadic combinator!
      */
    ...
    import com.fpinscala.ch13.FahrenheitToCelsius._
...
object ProcessApp extends App {
  ...
  println("------------------------------------------")
  println("> Running SimpleStreamTransducers.Process")
  println("------------------------------------------")
  //Process, filter:
